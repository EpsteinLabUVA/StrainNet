...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target, mask) in enumerate(self.data_loader):
            ...
            data, target, mask = augmentation(data, target, mask, self.device)
            ...
            for met in self.metric_ftns:
                self.train_metrics.update(met.__name__, met(output, target, mask))
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (data, target, mask) in enumerate(self.valid_data_loader):
                ...
                for met in self.metric_ftns:
                    self.valid_metrics.update(met.__name__, met(output, target, mask))
                # self.writer.add_image('input', make_grid(data.cpu(), nrow=8, normalize=True))
